...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const digitSmaller = Math.min(a, b);
  const digitBigger = Math.max(a, b);
  return `${startBracket}${digitSmaller}, ${digitBigger}${endBracket}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const result = Boolean(arr.reduce((sum, current) => sum + current, 0) % 10);
  return !result;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!(str[i] in dictionary)) {
      if (arr.length) {
        if (dictionary[arr[arr.length - 1]] === str[i]) {
          arr.pop();
        } else {
          return false;
        }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m2[0].length; j += 1) {
      result[i][j] = 0;
      for (let d = 0; d < m1[0].length; d += 1) {
        result[i][j] += m1[i][d] * m2[d][j];
      }
    }
  }
  return result;
